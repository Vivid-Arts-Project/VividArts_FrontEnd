          ...
          <div className="rounded-[23px] border border-white/[.08] bg-[#111025]/92 px-6 py-7 sm:px-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <div className="space-y-3">
                <label className="block"><span className={labelCls}>Password<span className="text-red-300"> *</span></span><span className={fieldCls}><Icon name="lock" size={17} className="text-white/30 group-focus-within:text-[#8da7ff]"/><input autoComplete="new-password" className="h-11 min-w-0 flex-1 border-none bg-transparent text-sm text-white outline-none placeholder:text-white/20" type={showPassword ? 'text' : 'password'} placeholder="Min 8 characters" value={form.password} onChange={e => set('password', e.target.value)} required/><button type="button" onClick={() => setShowPassword(value => !value)} aria-label={showPassword ? 'Hide password' : 'Show password'} className="flex h-8 w-8 items-center justify-center rounded-lg border-none bg-transparent text-white/30 cursor-pointer hover:bg-white/5 hover:text-white"><Icon name={showPassword ? 'eyeOff' : 'eye'} size={17}/></button></span><PasswordStrength password={form.password}/></label>
                <label className="block"><span className={labelCls}>Confirm password<span className="text-red-300"> *</span></span><span className={fieldCls}><Icon name="lock" size={17} className="text-white/30 group-focus-within:text-[#8da7ff]"/><input autoComplete="new-password" className="h-11 min-w-0 flex-1 border-none bg-transparent text-sm text-white outline-none placeholder:text-white/20" type={showConfirmPassword ? 'text' : 'password'} placeholder="Repeat password" value={form.confirmPassword} onChange={e => set('confirmPassword', e.target.value)} required/><button type="button" onClick={() => setShowConfirmPassword(value => !value)} aria-label={showConfirmPassword ? 'Hide confirm password' : 'Show confirm password'} className="flex h-8 w-8 items-center justify-center rounded-lg border-none bg-transparent text-white/30 cursor-pointer hover:bg-white/5 hover:text-white"><Icon name={showConfirmPassword ? 'eyeOff' : 'eye'} size={17}/></button></span></label>
